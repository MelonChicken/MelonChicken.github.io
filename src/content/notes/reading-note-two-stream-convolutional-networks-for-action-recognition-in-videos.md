...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3a414b84a0f280e4bb00ee5ce0365b3b.png)


### **3.1. Discriminative와 generative의 차이**

1. Discriminative model : 입력 x가 주어졌을 때 클래스 y를 예측하는 데 집중

    $P(y∣x)$

    > 예: 이 영상이 주어졌을 때 **행동이 달리기일 확률**은 얼마인가?
2. Generative model : 각 클래스의 데이터가 어떻게 생성되는지까지 모델링
...
각 네트워크는 마지막 softmax 층에서 행동별 점수를 독립적으로 출력한다.  ($s_x$) 그 후 이 점수들을 합쳐 최종 행동을 결정.


예를 들어 행동 클래스가 “달리기, 수영, 테니스” 세 개라고 설정할때,


공간 스트림의 출력:  $s_{\text{spatial}}=[0.2,\;0.1,\;0.7]$

> 공간 스트림은 라켓과 코트 같은 외형을 보고 `테니스`일 가능성을 높게 판단

시간 스트림의 출력: $s_{\text{temporal}} =[0.1,\;0.2,\;0.7]$
...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3a414b84a0f2808c9bffc1643ce68ca8.png)
...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3a514b84a0f2808481fedcb8a50079d2.png)
...
- 한 픽셀이 아니라 영상 전체에 계산하므로 **변위장(displacement field)**
    ...
    → **주어진 움직임 패턴이 어떤 행동을 뜻하는지**에 더 집중


그러면 optical flow를 추출했다는 가정 하에 어떻게 이를 입력값으로 모델에 넣는가?

> 이미 계산된 여러 flow의 $x,y$성분을 CNN의 $2L$개 채널로 쌓는다. 그럼 flow는 어떻게 읽어오는가?

![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3a514b84a0f280a6821ac7343b0276c1.png)


$d_t(u, v)$ : displacement vector at the point $(u, v)$ in the frame $t$


$\{d_t\} $: a set of displacement vector fields


$d^x_t, d^y_t$ : horizontal and vertical components of the vector field


> 💡 프레임 $t$의 좌표 $(u,v)$에 있는 영상 패턴이 다음 프레임 $t+1$에서 얼마나 이동했는지
...
> - $d_t^y(u,v)$: 수직 이동량
>
> e.g. $d_t(100,50)=(3,-2)$ : 좌표 $(100,50)$의 영상 패턴이 다음 프레임에서 대략 오른쪽으로 3픽셀, 위쪽으로 2픽셀 이동
...
카메라의 움직임을 고려해 각 $d$ (displacement field)에서 평균 벡터를 빼서 완화하는 **Mean flow subtraction** 전략도 존재한다
...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3a314b84a0f2803b8721d8cee29abd5b.png)
...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f28000bdd0e372d1eb538f.png)
...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f280899d23cf274fd12960.png)


![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f280adaf09d9c0b4b1a0e6.png)
...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f2802bb237feb30e2728b6.png)


→ 결과적으로 3번 선택지, multi-task learning formulation을 도입하는 것이 가장 성능이 높았다. (훈련 데이터를 전부 활용할 수 있었기 때문으로 추측됨.)


![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f2809786eec29873852eff.png)


### 5.3. Two-stream ConvNets


![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f280559c36e279f8716c3f.png)

> 두개의 스트림을 하나로 결합하는데 있어 가장 먼저 제시되는건 공간 네트워크와 시간 네트워크의 `full6` 또는 `full7` 층 위에 공동 fully connected layer들을 쌓아 학습하는 것.
>
> → 고차원 특징을 합친 뒤 새로운 fully connected layer들을 추가로 학습하는 방법을 시도했지만 과적합 문제로 인해 활용되지 않았다.
>
>
...
1. 애초에 두 스트림은 상호 보완적이었다 (합성시 두 개의 단일 스트림 모델에 대해 모두 성능이 향상되었기 때문)

    → 융합 모델이 Temporal stream 단독보다 약 `6%p`, Spatial stream 단독보다 약 `14%p` 높은 성능을 기록했다.
...
![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f280f2b9d1c05725ae978b.png)

> UCF-101, HMDB-51 데이터셋에 대한 SOTA 모델들과 성능을 비교해본 결과, 경쟁 가능한 수준을 보여주었다

![Notion image](/notion-assets/reading-note-two-stream-convolutional-networks-for-action-recognition-in-videos/3aa14b84a0f2806a9d2dc6adb4f3d162.png)
...
> 이 논문의 Temporal ConvNet은 trajectory stacking을 통해 궤적을 따라 optical flow를 입력으로 구성하지만, **네트워크 내부의 spatial pooling은 궤적을 고려하지 않는다**는 한계가 있다.
>
> 또한 카메라 움직임을 **평균 변위 벡터 차감**으로만 단순하게 보정하므로, 복잡한 전역 카메라 모션을 충분히 제거하지 못한다. 따라서 향후에는 trajectory-aligned spatio-temporal pooling과 명시적인 camera motion compensation이 필요하다.
>
>
