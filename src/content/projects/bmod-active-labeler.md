...
2. Gemini가 각 영상 구간에 대해 행동 라벨 후보와 시각적 관찰 근거를 제안하도록 한다.
3. 사용자가 AI 제안을 수용, 수정, 기각하거나 불확실 구간으로 분류할 수 있도록 한다.
4. AI 최초 제안과 인간 최종 판단을 모두 기록하여 AI 오류 유형을 분석할 수 있도록 한다.
5. 수동 라벨링 대비 AI 보조 라벨링이 처리 시간과 판단 부담을 줄이는지 평가한다.
6. 생성된 CSV를 후속 행동 분류 모델 학습 파이프라인에서 바로 사용할 수 있도록 한다.
7. 향후 pseudo-labeling, active learning, pose/keypoint sequence 분석으로 확장 가능한 기반을 만든다.
