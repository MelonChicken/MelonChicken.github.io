...
{/* Unsupported Notion block type: child_database (32414b84-a0f2-80a5-a97f-f664f9eccb6d) */}

{/* Unsupported Notion block type: table_of_contents (36c14b84-a0f2-80dc-bfa0-f228e325e807) */}
...
- 구현 범위 \(Implementation Scope\)
  ...
  - **지식 추출 및 그래프 구축 \(Graph Construction\)**:
    ...
    - LLM을 이용해 각 청크에서 개체\(Entity\)와 관계\(Relation\)를 추출하여 트리플\(N\_\{s\}, R, N\_\{t\}\)을 생성하는 모듈을 개발합니다\.
...
- 성능 평가 기준
  ...
  - **문맥 정밀도 \(Context Precision\) **→ 상위 검색 결과 중 **관련 있는 문맥이 얼마나 정확하게 포함되었는지** 평가
    \\frac\{\\sum\_\{k=1\}^\{K\} Precision@k \\times v\_k\}\{\\text\{Total num of relevant items in the top K results\}\}
  ...
  - **답변 관련도 \(Answer Relevancy\) **→ 생성된 답변이 **질문과 의미적으로 얼마나 관련 있는지** 평가
    \\frac\{1\}\{N\}\\sum\_\{i=1\}^\{N\}\\cos\(E\_\{g\_i\}, E\_o\)
...
1. **HybridRAG: Integrating Knowledge Graphs and Vector Retrieval Augmented Generation for Efficient Information Extraction **[https://arxiv\.org/pdf/2408\.04948](https://arxiv.org/pdf/2408.04948)

1. ARAGOG Dataset [https://github\.com/predlico/ARAGOG](https://github.com/predlico/ARAGOG)
